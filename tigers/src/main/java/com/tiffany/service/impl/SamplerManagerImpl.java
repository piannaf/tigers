...
public class SamplerManagerImpl extends GenericManagerImpl<Sampler, Long>
	implements
    ...
    public List<Sampler> getMySamplers(User user) {
	/* This assumes that each user has only one role; simply chooses the 
	 * last one
	 */
	String myRole = "";
	...
	if (myRole.equals("ROLE_CONTRACTOR")) {
	    return samplerDao.findByContractor(user);
	} else if (myRole.equals("ROLE_LABORATORY")) {
	    return samplerDao.findByLaboratory(user);
	} else {
