...
<c:set var="title_key" value="Sampler List for Audit Report"/>
<c:set var="page_description" value="Select a sampler from the map below to view environmental audit reports for the sampler."/>
<c:set var="target_url" value="reportsamples.html"/>
        ...
        function initialize() {    
            ...
            GEvent.addListener(mmap, 'click', function(marker) {
                if(marker && marker.getIcon) {    // check whether it's really a marker
                        var html = "reportsamples.html?tag=" + marker.title;
