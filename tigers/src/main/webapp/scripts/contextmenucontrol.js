...
function ContextMenuControl(opt_opts) {
  if (opt_opts) {
    this.options = {
      ...
      addSampler: opt_opts.addSampler	//JM
    };
  } else {
    this.options = {};
  }
...
ContextMenuControl.prototype.createContextMenu_ = function (change) {
  ...
  if (me.options.addSampler !== false) {	// JM
	    me.menuList.appendChild(me.createListItem_("Add Sampler here", "addSampler"));
	  }
  me.hideMenu_();
...
/**
 * Sends user to the add sampler view with lat/lng filled in.
 */
ContextMenuControl.prototype.goToAddSampler_ = function () {
	var me = this;
	var lat = me.clickedPoint_.lat().toFixed(6);
	var lng = me.clickedPoint_.lng().toFixed(6);
	
	window.location = "officer/samplerform.html?lat=" + lat + "&lng=" + lng;
};