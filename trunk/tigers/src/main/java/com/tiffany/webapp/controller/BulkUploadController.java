...
public class BulkUploadController extends BaseFormController {
	...
	User remoteUser = userManager.getUserByUsername((request
		.getRemoteUser()));
	List<String> samplerIdList = samplerManager
		.getTagListForLaboratory(remoteUser);
	refData.put("samplerIdList", samplerIdList);
	if (samplerIdList.size() == 0)
	    saveMessage(request, getText("bulkUpload.form.noTag", locale));
	return refData;
    }
	...
	// invalid header
	if (!bulkUploadService.initService(type, line,
		samplerManager.getByTag(samplerId), userManager.getUserByUsername(request.getRemoteUser()))) {
	    if (type.equals("G"))
		type = "Ground Water";
	    else
		type = "Surface Water";
	    saveError(request,
		    getText("errors.bulkUpload.header", type, locale));
	    text.close();
	    csv.delete();
	    return showForm(request, response, errors);
	}
	...
	while (it.hasNext()) {
	    ...
	    log.debug("item " + key + " : "
		    + bulkUploadService.fieldsMap.get(key));
	}
	// =======================
	int count = 1;
