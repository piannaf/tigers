...
public class AssignLabPostController extends BaseFormController {
    ...
    // not officer not adding
    	...
    	if (request.getParameter("tag") == null) {
    		...
    		Sampler sampler = samplerManager.getByTag(request.getParameter("tag"));
    		if (!samplerList.contains(sampler)) {
    			response.sendError(HttpServletResponse.SC_FORBIDDEN);
        		log.warn("User '" + request.getRemoteUser() + "' is trying to assign others' sampler to laboratory.");
        		throw new AccessDeniedException("You do not have permission to assign this sampler.");
    		}
    	}
	...
	protected Object formBackingObject(HttpServletRequest request) throws Exception {
    	...
    	if (samplerId != null) assignLab.setSamplerId(samplerId);
    	return assignLab;
    }
	//=======================referenceData=============================================
    protected Map referenceData(HttpServletRequest request, Object command, Errors errors) throws Exception {
		...
		if (samplerId != null) sampler = samplerManager.getByTag(samplerId);
		...
		if (sampler != null) referenceData.put("sampler", sampler);
    ...
    public ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command,
    	...
    	log.debug("request: assign sampler '" + samplerId + " ' to user '" + labname + " '");
    	Sampler sampler = samplerManager.getByTag(samplerId);
		User lab = null;
		try {lab = userManager.getUserByUsername(labname);}
